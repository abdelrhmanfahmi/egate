            ...
            <div v-for="(input, index) in dynamicInputs" :key="index" class="newForm">
                <b-form-group>
                    <div v-if="input.type == 'text'">
                        <label :for="input.uuid">{{ $i18n.locale == 'en' ? input.title_en :
                            input.title_ar }}</label>
                        <span class="requried tetx-danger" v-if="input.required">*</span>
                        <b-form-input type="text" :id="input.uuid" v-model="form[input.uuid]"
                    ...
                    <div v-if="input.type == 'checkbox'">
                        <b-form-checkbox v-model="form[input.uuid]" :value="true" :unchecked-value="false"
                            :class="{ 'dynamic-input': cartPage }">
                            <span>{{ $i18n.locale == 'en' ? input.title_en : input.title_ar
                            }}</span> <span class="requried tetx-danger" v-if="input.required">*</span>
                        </b-form-checkbox>
                        <div class="error" v-for="(error, index) in errors[input.uuid]" :key="index">
                            {{ error }}
                        </div>
                    ...
                    <div v-if="input.type == 'file'">
                        <div class="row justify-content-center align-items-center">
                            <div
                                :class="{ 'col-md-8 col-sm-12': imagesPaths[input.uuid], 'col-12': !imagesPaths[input.uuid] }">
                                <label :for="imagesPaths[input.uuid]">{{ $i18n.locale == 'en' ? input.title_en :
                                    input.title_ar
                                }}</label>
                                <span class="requried tetx-danger" v-if="input.required">*</span>
                                <b-form-group>
                            ...
                            <div
                                :class="{ 'col-md-4 col-sm-12': imagesPaths[input.uuid], 'col-12': !imagesPaths[input.uuid] }">
                                ...
                                <div class="" v-if="imagesPaths[input.uuid]">
                                    ...
                                    ">
                                        <img v-b-modal.dynamicFile :src="imagesPaths[input.uuid]" alt="dynamic-file-image"
                                        ...
                                        <b-modal id="dynamicFile" title="image">
                                            ...
                                            <template>
                                                <img :src="imagesPaths[input.uuid]" alt="moa-image"
                                                    v-if="imagesPaths[input.uuid]" class="img-fluid w-100" />
                                            </template>
                                            ...
                                            <template #modal-footer>
                                                <b-button class="btn-block" variant="outline-success" @click="
                                                    downloadImage(
                                                        ...
                                                        (extension = imagesPaths[input.uuid]
                                                            .split('.')
                                                            .pop()),
                                                        $t('profile.commercialLicense')
                                                    )
    ...
    methods: {
        ...
        */
        ...
        async secondCheckDynamicInputs() {
            await profile.getDynaimicInputsData().then(res => {
