            ...
            <div class="slider text-center">
                <VueSlickCarousel v-bind="settings" v-if="items && items.length" class="my-2">
                    <div v-for="(item, index) in items" :key="index" class="slide-holder">
                        ...
                        <div class="row" :class="{ 'text-left': $i18n.locale == 'en', 'text-right': $i18n.locale == 'ar' }">
                            <div class="col-xl-4 col-lg-12 col-md-12 col-sm-12">
                                
                                <div class="time-holder" v-if="item.current_main_image_path !== null">
                                    <b-img :src="item.current_main_image_path" class="offer-image w-100"> </b-img>
                                    <div class="time-count" v-if="item.offer_active_by_type.end_date">
                                    <Countdown :until="item.offer_active_by_type.end_date" format="DHMS"
                                        wrap="coming-countdown countdown-separator"></Countdown>
                                </div>
                                </div>
                            </div>
                            <div class="col-xl-8 col-lg-12 col-md-12 col-sm-12">
                                ...
                                <div class="row mb-3 align-items-center">
                                    <div class="col-md-6 col-sm-12" v-if="item.product_details_by_type.price">
                                        <h6 class="main-color">{{ item.product_details_by_type.price | fixedCurrency }} {{
                                            currency }}</h6>
                                    </div>
                                    <div class="col-md-6 col-sm-12"
                                        v-if="item.product_details_by_type.price_before_discount">
                                        <del>
                                            <h6>{{ item.product_details_by_type.price_before_discount | fixedCurrency }}
                                                {{ currency }}</h6>
                                        </del>
                                    </div>
                                </div>
                                <!-- supplier  & weight  -->
                                <div class="row mb-3 align-items-center">
                                    <div class="col-md-6 col-sm-12" v-if="item.client.id && item.client.company">
                                        <router-link :to="`/suppliers/${item.client.id}`"
                                            class="supplier-name">
                                            <h6>{{ $t('items.supplier') }} : {{
                                                item.client.company
                                            }}</h6>
                                    ...
                                    <div class="col-md-6 col-sm-12" v-if="item.product_details_by_type.weight">
                                        <h6>{{ $t('singleProduct.weight') }} : {{ item.product_details_by_type.weight }}
                                        </h6>
                                    </div>
                                </div>
                                <!-- product category -->
